...
def test_ai_governance_pipeline_duration(test_config, spark_session):
    ...
    spark_session.createDataFrame([{"warmup": 1}]).count()

    # Start timer
    start_perf_time = time.perf_counter()

    ai_governance_main(table_name, test_config, spark_session)
    time.sleep(0.5)

    # End timer
    total_perf_time = time.perf_counter() - start_perf_time

    test_logger.info("\n" + "=" * 25 + " [PERFORMANCE] AI CLASSIFICATION PIPELINE DURATION BENCHMARK RESULT " + "=" * 25)
